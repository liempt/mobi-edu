...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(window).scroll(function () {
            ...
            if (old_scroll != scroll || old_scroll == 0) {
                ...
                old_scroll = scroll;
                if ($(this).scrollTop() >= 1000) {        // If page is scrolled more than 1200x
        ...
        $(body).on("click", ".maso-list .menu-inner a", function () {
            ...
            //    setTimeout(function () {
            //        var i = 0;
            //        var num = parseInt($(lm).attr("data-page-items"), 3);
    ...
    $.fn.initIsotope = function () {
        if ($.isFunction($.fn.isotope)) {
            ...
            var $container = $(m).isotope(options);
            if ($.isFunction($.fn.renderLoadedImgs)) {
            ...
            function loadMore(toShow) {
                ...
                setTimeout(function () { jQuery(window).trigger('resize.px.parallax'); }, 300);
            }

            //append load more button
            $container.after('<button id="load-more" class="btn-text active">Xem th\u00eam </button>');

            //when load more button clicked
            $("#load-more").click(function () {
                if ($(this).closest(".maso-box").data('clicked')) {
                    //when filter button clicked, set initial value for counter
                    counter = initShow;
                    $(this).closest(".maso-box").data('clicked', false);
                } else {
                    counter = counter;
                };
            ...
            var $filters = $(".maso-list .menu-inner ul li a").click(function () {
                // set filter in hash
                let datafilter = $(this).attr('data-filter');
                location.hash = 'filter=' + encodeURIComponent(datafilter);

            });
            //var firstload = true ;
            function onHashchange() {
                var hashFilter = getHashFilter();
                let filter = hashFilter;
                if (hashFilter == "*" || hashFilter == null) {
                    hashFilter = $(".maso-list .menu-inner ul li a:first").attr('data-filter');
                }
                //console.log(filter);
                if (hashFilter) {
                    $(".maso-list .menu-inner ul li").removeClass("active");
                    $(".maso-list .menu-inner ul li").find('[data-filter="' + hashFilter + '"]').parent("li").addClass("active");
                    if (!(filter == "" || filter == null)) {
                        setTimeout(function () {
                            $('html, body').animate({ scrollTop: $("#section-solution").offset().top }, 400);
                        }, 150);
                    }
                }
                let active = $(nav).hasClass("active");
                ...
                $(this).closest(".maso-box").data('clicked', true);
                loadMore(initShow);
            }
